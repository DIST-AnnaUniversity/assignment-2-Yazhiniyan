...
s3 = boto3.client('s3',aws_access_key_id = "AKIAZJAQWYAO7ADUUNGI",aws_secret_access_key = "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")

# Postgresql Database Connection
conn = psycopg2.connect(host='localhost',database='hostel1',user='postgres',password='kabi')
...
@app.route('/upload',methods=['POST']) #Upload Photo - Admin
def upload_file():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("select name from admin where roll=%s",(file_name,))
        res = cursor.fetchall()
        if res:
            flash('Not Uploaded','info')
            return render_template('upload.html',messages='Not uploaded')
        cursor.execute("select roll from photos where data=%s",(file_data,))
        res = cursor.fetchall()
        if res:
            flash('Not Uploaded','info')
            return render_template('upload.html',messages='Not uploaded')
        ...
        cursor.execute("insert into student_attendance(roll,total_sessions,time,date) values(%s,%s,%s,%s)",(file_name,0,current_datetime-timedelta(days=1),current_date-timedelta(days=1)))
        conn.commit()
...
@app.route("/questions",methods=["POST"]) #Home to Send Query page
def questions():
...
@app.route("/sendtoadmin",methods=['POST']) # Send query to the admin
def send_to_admin():
...
@app.route("/viewquery",methods=['POST']) #View Query - Students
def view_query():
    roll=request.form['roll']
    name=request.form['name']
    cursor=conn.cursor()
...
@app.route("/viewtaken") #Query taken by employee
def viewtaken():
    cursor = conn.cursor()
...
@app.route("/reactquery") # Complete queries - Admin
def reactquery():
    cursor=conn.cursor()
    cursor.execute("select roll,query,response from query where response='Not Completed'")
    res=cursor.fetchall()
...
@app.route("/searchemp",methods=['POST']) # Search Employee - Admin
def searchemp():
    name=request.form['name']
    cursor = conn.cursor()
...
@app.route('/searchdata', methods=['GET', 'POST']) # Search Student - Admin
def searchdata():
    if request.method == 'POST':
        ...
        if rollno!="" and hostel=="":
            ...
            print(results)
            cursor.execute("select bill,room,water,electricity from bill_needs where roll=%s",(rollno,))
            res=cursor.fetchone()
            bill = tuple()
            if res:
                bill = (res[0]+res[1]+res[2]+res[3],)
            ...
            if results:
                for i in results:
                    cursor.execute("select bill,room,water,electricity from bill_needs where roll=%s",(i[2],))
                    val=cursor.fetchone()
                    if val:
                        bill+=(val[0]+val[1]+val[2]+val[3],)
                    print(val,bill)
...
@app.route('/complete',methods=['POST']) # Uncompleted queries shown for students
def complete():
    roll=request.form['roll']
    query=request.form['query']
    cursor=conn.cursor()
...
@app.route('/payfees',methods=['POST']) #Pay fees 
def payfees():
    roll = request.form['roll']
    name=request.form['name']
    cursor = conn.cursor()
...
@app.route("/pay",methods=['POST']) #Pay fees page to Home Page
def pay():
    roll = request.form['roll']
    name = request.form['name']
...
@app.route("/upbill",methods=["POST"]) #Update bill for all the students
def upbill():
...
@app.route("/home",methods=['POST']) #Home page
def home():
    roll = request.form['roll']
    name = request.form['name']
    print(roll,name)
...
def save_receipt(receipt):
    ...
    # Modify the file path with the date string
    modified_receipt_file = f"{date_string}.pdf"
    if receipt_file:
        s3.upload_file(receipt_file,'yazhhostelmanagement',modified_receipt_file)
        print('File uploaded successfully!')
    webbrowser.open(receipt_file)
    print(f"Fee receipt generated: {modified_receipt_file}")

@app.route("/generatepdf",methods=["POST"]) #Genarate Pdf
def generatepdf():
...
@app.route("/update",methods=['POST']) #Update Photo
def update():
    file = request.files['file']
    file_name = request.form.get('name')
    print(file_name)
...
@app.route("/encode",methods=["POST"]) #Encode Code
def encode():
    print("encode")
...
@app.route('/mess',methods=['POST']) #Mess Fees Page
def mess():
    name=request.form['name']
    roll=request.form['roll']
    conn = psycopg2.connect(host="localhost", database="hostel1", user="postgres", password="kabi")
